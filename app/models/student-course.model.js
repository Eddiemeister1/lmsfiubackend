...
StudentCourse.findByCourseId = (courseId, result) => {
    return new Promise((resolve, reject) => {
        sql.query("SELECT * FROM studentsincourses WHERE course_id = ? ORDER BY l_name, f_name", [courseId], (err, res) => {
            if (err) {
                result(err, null);
                return reject(err);
            }
...
StudentCourse.getAvgPts = (courseId, studentId, result) => {
    return new Promise((resolve, reject) => {
        sql.query("CALL getAvgPointsOfClass(?)", [courseId], (err, res) => {
            if (err) {
                result(err, null);
                return reject(err);
            }
...
StudentCourse.getStudentPoints = (courseId, studentId,result) => {
    return new Promise((resolve, reject) => {
        sql.query("CALL getStudentPointsInCourse(?, ?)", [courseId, studentId], (err, res) => {
            if (err) {
                result(err, null);
                return reject(err);
            }
...
StudentCourse.getAllPossiblePoints = (courseId, result) => {
    return new Promise((resolve, reject) => {
        sql.query("CALL getTotalPossiblePoints(?)", [courseId], (err, res) => {
            if (err) {
                result(err, null);
                return reject(err);
            }
