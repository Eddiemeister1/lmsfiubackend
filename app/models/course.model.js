...
Course.findGroupById = (courseId, result) => {
    return new Promise((resolve, reject) => {
        sql.query("SELECT tgroup FROM courses WHERE id = ?", [courseId], (err, res) => {
            if(err) {
                result(err, null);
                return reject(err);
            }
...
Course.updateById = (id, course, result) => {
    return new Promise((resolve, reject) => {
        sql.query("UPDATE courses SET name = ?, description = ?, seats = ?, tgroup = ?, start_date = ?, end_date = ? WHERE id = ?",
            [course.name, course.description, course.seats, course.tgroup, course.start_date, course.end_date, id], (err, res) => {
                if(err) {
                    result(err, null);
                    return reject(err);
                }
...
Course.updateGroupsById = (id, course, result) => {
    return new Promise((resolve, reject) => {
        sql.query("UPDATE courses SET tgroup = ? WHERE id = ?",
            [course.tgroup, id], (err, res) => {
                if(err) {
                    result(err, null);
                    return reject(err);
                }
