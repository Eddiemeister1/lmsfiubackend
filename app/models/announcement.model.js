...
Announcement.findByCourseIdAndCurrentUserId = (info, result) => {
    return new Promise((resolve, reject) => {
        sql.query("SELECT * FROM inbox WHERE course_id = ? AND (type='C' or ((sender_id = ? OR receiver_id = ?) AND parent_id is null)) OR type = 'G' ORDER BY changed DESC", 
        [info.courseId, info.userId, info.userId], (err, res) => {
            if(err) {
                result(err, null);
                return reject(err);
            }
...
Announcement.findByCourseId = (courseId, result) => {
    return new Promise((resolve, reject) => {
        sql.query("SELECT * FROM inbox WHERE (course_id = ? AND parent_id is null) OR type = 'G' order by changed DESC", 
        [courseId], (err, res) => {
            if(err) {
                result(err, null);
                return reject(err);
