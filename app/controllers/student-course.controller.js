...
exports.getStudentPoints = (req, res) => {
    StudentCourse.getStudentPoints(req.params.courseId, req.params.studentId, (err, data) => {
        if (err) {
            res.status(500).send({
                message: err.message || "Error occured while retriving student points in class"
            });
        } else {
            res.send(data);
        }
...
exports.getAllPossiblePoints = (req, res) => {
    StudentCourse.getAllPossiblePoints(req.params.courseId, (err, data) => {
        if (err) {
            res.status(500).send({
                message: err.message || "Error occured while retriving all possible points in course"
            });
        } else {
            res.send(data);
        }
