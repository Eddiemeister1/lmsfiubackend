...
exports.findGroups = (req, res) => {
    Course.findGroupById(req.params.courseId, (err, data) => {
        if(err) {
            if(err.kind == "not_found"){
                res.status(404).send({
                    message: `Not found Course with id ${req.params.courseId}.`
...
// Update a Course's group identified by the courseId in the request
exports.updateGroups = (req, res) => {
    ...
    Course.updateGroupsById(req.params.courseId, new Course(req.body), (err, data) => {
        if(err) {
            if(err.kind == "not_found") {
                res.status(404).send({
                    message: `Not found Course with id ${req.params.courseId}.`
