...
exports.findByCourseIdAndCurrentUserId = (req, res) => {
    ...
    Announcement.findByCourseIdAndCurrentUserId(reqParams, (err, data) => {
        if(err) {
            if(err.kind == "not_found"){
                res.status(404).send({
                    message: `Not found ${className} with id ${reqParams}.`
                });
            } else {
                res.status(500).send({
                    message: `Error retrieving ${className} with id ${reqParams}`
                });
            }
        } else {
            res.send(data);
        ...
        console.log(`${className} findById(${reqParams}) was found`);
    }).catch((err) => {
        console.log(`Error findById(${reqParams}), couldn't find/retrieve ${className}\n${err}`);
    })
};
